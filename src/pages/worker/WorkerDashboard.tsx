...
const WorkerDashboard: React.FC = () => {
  ...
  // Fetch stats and bookings on mount
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch stats
        const statsResponse = await axios.get('/api/workers/me/stats', {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        });
        setStats([
          {
            label: 'Total Earnings',
            value: `$${statsResponse.data.data.totalEarnings}`,
            change: '+12%',
            icon: DollarSign,
            color: 'green'
          },
          {
            ...
            value: statsResponse.data.data.completedJobs,
            change: '+8%',
            icon: CheckCircle,
            color: 'blue'
          },
          {
            ...
            value: statsResponse.data.data.averageRating.toFixed(1),
            change: '+0.1',
            icon: Star,
            color: 'yellow'
          },
          {
            label: 'Response Rate',
            value: `${statsResponse.data.data.responseRate}%`,
            change: '+2%',
            icon: MessageSquare,
            color: 'purple'
          }
        ...
        // Fetch bookings
        const bookingsResponse = await axios.get('/api/workers/bookings/me', {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        });
