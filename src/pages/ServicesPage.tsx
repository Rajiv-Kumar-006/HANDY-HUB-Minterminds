...
const ServicesPage: React.FC = () => {
  ...
  const filteredServices = services.filter(service => {
    ...
    const avgPrice = (service.basePrice.min + service.basePrice.max) / 2;
    const matchesPrice =
      priceRange === 'all' ||
      (priceRange === 'low' && avgPrice < 75) ||
      (priceRange === 'medium' && avgPrice >= 75 && avgPrice <= 120) ||
      (priceRange === 'high' && avgPrice > 120);
  ...
  const sortedServices = [...filteredServices].sort((a, b) => {
    switch (sortBy) {
      ...
      case 'price-low':
        return (a.basePrice.min + a.basePrice.max) / 2 - (b.basePrice.min + b.basePrice.max) / 2;
      case 'price-high':
        return (b.basePrice.min + b.basePrice.max) / 2 - (a.basePrice.min + a.basePrice.max) / 2;
      case 'reviews':
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {sortedServices.map(service => (
              <div key={service._id} className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-200">
                <img
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="text-2xl font-bold text-gray-900">
                        ${service.basePrice.min} - ${service.basePrice.max}
                      </span>
