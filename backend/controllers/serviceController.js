...
exports.createService = async (req, res, next) => {
  try {
    const { title, description, category, provider, price, duration, location } = req.body;
    let imageUrl = '';

    // Handle image upload if file is provided
    if (req.file) {
      const result = await cloudinary.uploader.upload_stream({
        folder: 'handyhub/services',
      }, (error, result) => {
        if (error) {
...
exports.getCategories = async (req, res, next) => {
  try {
    ...
    // Calculate active workers per category (using unique providers)
    const categoryStats = await Promise.all(
      categories.map(async (category) => {
        const workerCount = await Service.distinct('provider', { category: category.id }).countDocuments();
        return { ...category, activeWorkers: workerCount };
      })
    );
